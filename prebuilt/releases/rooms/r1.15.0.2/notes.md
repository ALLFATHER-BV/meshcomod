...
**Train:** **`r1.15.0.2`**. On-device version string: **`meshcomod-r1.15.0.2-room-mt-<gitsha>`**.
...
**Highlights (vs r1.15.0.1):**
- **USB Wi‑Fi CLI parity with repeater/companion:** `set wifi.ssid`, `set wifi.pwd`, `wifi.apply`, `wifi.clear`, `get wifi.ssid`, `set wifi.radio`, `get wifi.status` / `wifi.status` (NVS runtime store + immediate STA re-apply when the radio is turned on).

**Protocol surface:** Unchanged from r1.15.0.1 — handshake only (`CMD_DEVICE_QUERY` / `CMD_APP_START` shape); framed sync/contacts/chat client commands return **`RESP_CODE_ERR`** + **`ERR_CODE_UNSUPPORTED_CMD`**.

**Build:** `export ROOM_FIRMWARE_VERSION=r1.15.0.2-room-mt` → `sh build.sh build-room-multitransport-firmwares` → `sh scripts/copy-room-release-bins.sh r1.15.0.2`.
...
**Wi‑Fi:** Build still uses `scripts/inject_wifi_env.py` for optional compile-time **`WIFI_SSID`** / **`WIFI_PWD`**. Runtime credentials are stored in NVS (meshcomod companion path or the USB commands above).
