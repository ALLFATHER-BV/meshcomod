...
- **Heltec V4 TFT geometry fix:** the V4 TFT base now uses portrait logical mapping (`DISPLAY_ROTATION=2`, swapped width/height scaling), so UI layout is no longer too wide/short after the rotation change.
- **Primary companion prebuilts only in this folder:** V4 OLED USB+TCP, V4 TFT+touch USB+TCP, and V3 USB+TCP.
...
**Build:** `DISABLE_DEBUG=1`, `FIRMWARE_VERSION=v1.14.1.2`.
